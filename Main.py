...
main = tkinter.Tk()
main.title("Detection of bot Using Graph-Based Machine Learning") #designing main screen
...
def decisionTree():
    ...
    for i in range(len(y_pred)):
        ...
        if y_pred[i] == 0 and Y_test[i] != y_pred[i]:
           FN += 1
    text.insert(END,'True Positive  : '+str(TP)+"\n")
    text.insert(END,'False Positive : '+str(FP)+"\n")
    text.insert(END,'True Negative  : '+str(TN)+"\n")
    text.insert(END,'False Negative : '+str(FN)+"\n")
...
font = ('times', 16, 'bold')
title = Label(main, text='Detection of bot Using Graph-Based Machine Learning')
